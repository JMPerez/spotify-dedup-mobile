...
import SpotifyWebApi from './api/index';
import promisesForPages from './api/promiseForPages';
...
export default class Deduplicator {
  ...
  async removeDuplicates(playlist) {
    return new Promise((resolve, reject) => {
      ...
      if (playlist.collaborative) {
        ...
        this.promiseThrottle.add(async () => {
          const tracksToRemove = playlist.duplicates.map(d => ({
